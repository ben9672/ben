				...
				if (CurrentMenuItemFunc.AdjustFunction)
				{
					...
					if (CurrentMenuItemFunc.AdjustFunction(menuAction))
					{
						...
						{	//加上√号部分的内容，可以解决进入LanguageMenu三级菜单时出现白框的问题	
							...
							Osd_Set256TextColor( 0x03, Color_2);
							Osd_DrawContinuesChar( SUB_TEXT_XPOS, ( SUB_TEXT_YPOS + OSD_LANGUAGE_ITEM     * IconShift ), SpaceFont, 27 );	
							Osd_DrawContinuesChar( SUB_TEXT_XPOS, ( SUB_TEXT_YPOS + OSD_HPOSITION_ITEM    * IconShift ), SpaceFont, 27 );
							Osd_DrawContinuesChar( SUB_TEXT_XPOS, ( SUB_TEXT_YPOS + OSD_VPOSITION_ITEM    * IconShift ), SpaceFont, 27 );
							Osd_DrawContinuesChar( SUB_TEXT_XPOS, ( SUB_TEXT_YPOS + OSD_TIMEOUT_ITEM      * IconShift ), SpaceFont, 27 );
							Osd_DrawContinuesChar( SUB_TEXT_XPOS, ( SUB_TEXT_YPOS + OSD_TRANSPARENCE_ITEM * IconShift ), SpaceFont, 27 );
							//重画二级菜单------------------------------						√
...
//--------------------------- 显示当前整个OSD界面 ---------------------------------									√
	...
	/*else if((MenuPageIndex >= BrightnessMenu && MenuPageIndex <= TransparenceMenu) || MenuPageIndex == HotInputSelectMenu)
	{
		int i;
		DrawOsdBackGround();	//刷新二、三级界面
		for(i=0; i<MenuItemCount; i++)
	...
	else
	{
		...
		if (redrawFlags)
		{
			...
			else
			{
				...
				{
					Osd_SetPosition(50, 50);			//工厂模式下，菜单界面的坐标       				√
				}
		...
		while( 1 )
		{
			if( drawItem->DisplayText )
			{
				...
				else if(MenuPageIndex == MainMenu)
				{
					if(itemIndex == MenuItemIndex)
					{
						Osd_Set256TextColor( SUB_BACKCOLOR, Color_2);
					}
					else
					{
						Osd_Set256TextColor( SUB_FORCECOLOR, Color_2);
					}
...
//----------------------------------- 画主菜单背景 -----------------------------------
	if ( MenuPageIndex == MainMenu) 		 
	{
		...
		for (i = 4; i <= OsdWindowHeight - 5; i++)		//从此坐标开始画大背景，还可以解决换菜单时出现页面闪动的问题
		{
	...
	//// 二级菜单背景 ////
	//////////////////////
	else if((MenuPageIndex >= OSD_BriContrastMenu && MenuPageIndex <= ExitMenu))
	{
		Osd_SetTextMonoColor(BLACK_COLOR, BLACK_COLOR);	//大背景
		for (i = 4; i <= OsdWindowHeight - 5; i++)		//从此坐标开始画大背景，还可以解决换菜单时出现页面闪动的问题
		{
